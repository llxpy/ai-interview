...
export default function App() {
  const {
    ...
    loadCategory, getQuestions, toggleDone, toggleStar, resetProgress,
  } = useQuestions()
  ...
  // Auto-load first category on startup
  useEffect(() => {
    if (!loading && catIndex.length > 0 && currentCat === "") {
      const firstCat = catIndex[0].name
  ...
  // Get current questions from cache
  const questions = useMemo(() => getQuestions(currentCat), [getQuestions, currentCat, categoryData])
  ...
  const filtered = useMemo(() => {
    ...
    return qs
  }, [questions, search, diffFilter, showStarred, showDoneOnly, starSet, doneSet])
  ...
  // Loading state
  const isLoadingCat = currentCat === "" || loadingCat
      ...
      <div className="min-h-screen">
        <div className="h-14 sm:h-16 glass border-b border-border/30 flex items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-2.5">
            ...
            <div><div className="w-24 h-4 skeleton mb-1.5" /><div className="w-16 h-2.5 skeleton" /></div>
          </div>
          <div className="w-8 h-8 rounded-lg skeleton" />
        </div>
        <div className="max-w-[1440px] mx-auto flex">
          <div className="hidden lg:block w-[232px] p-3 space-y-2">
            {Array.from({ length: 10 }, (_, i) => <div key={i} className="h-8 rounded-lg skeleton" style={{ width: `${60 + Math.random() * 40}%` }} />)}
          </div>
          <div className="flex-1 p-3 sm:p-5 space-y-2.5">
            {Array.from({ length: 8 }, (_, i) => <div key={i} className="h-16 rounded-xl skeleton" />)}
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center max-w-sm">
          ...
          <button onClick={() => location.reload()} className="mt-4 px-6 py-2.5 bg-primary text-primary-foreground rounded-xl font-medium">刷新页面</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex flex-col pb-16 lg:pb-0">
      <Header total={totalCount} done={doneCount} starred={starCount} percent={percent} theme={theme} onToggleTheme={toggleTheme} />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 sm:gap-3">
              <div className="flex items-center gap-2.5 flex-1 min-w-0">
                <div className="flex bg-card rounded-xl p-1 border border-border/50 shrink-0">
                  <button onClick={() => setMode("list")} className={cn("flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all", mode === "list" ? "bg-primary/15 text-primary glow-border" : "text-muted-foreground hover:text-foreground")}>
                    <LayoutList className="w-4 h-4" /> 列表
                  </button>
                  <button onClick={startQuiz} className={cn("flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all", mode === "quiz" ? "bg-primary/15 text-primary glow-border" : "text-muted-foreground hover:text-foreground")}>
                    <Target className="w-4 h-4" /> 刷题
                  </button>
                </div>
                <div className="flex-1 relative min-w-0">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                  <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} placeholder="搜索题目..."
                    className="w-full pl-9 pr-3 py-2 sm:py-2.5 bg-card border border-border/50 rounded-xl text-xs sm:text-sm text-foreground placeholder:text-muted-foreground/60 outline-none transition-all focus:border-primary/50" />
                </div>
              </div>
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <div className="flex gap-0.5">
                  {DIFF_FILTERS.map((d) => (
                    <button key={d.v} onClick={() => { setDiffFilter(d.v); setPage(1) }}
                      className={cn("min-w-[28px] h-7 sm:min-w-[32px] sm:h-8 rounded-md border text-[9px] sm:text-[10px] font-medium flex items-center justify-center transition-all px-1",
                        diffFilter === d.v ? "bg-primary/15 border-primary/40 text-primary" : "bg-card border-border/50 text-muted-foreground hover:border-primary/30")}>
                      {d.l}
                    </button>
                  ))}
                </div>
                <button className="lg:hidden flex items-center gap-1.5 px-3 py-2 bg-card border border-border/50 rounded-xl text-xs text-muted-foreground transition-all hover:border-primary/30" onClick={() => setDrawerOpen(true)}>
                  <Filter className="w-3.5 h-3.5" />
                  <span className="max-w-[60px] truncate">{currentCat === "全部" ? "分类" : currentCat}</span>
                </button>
                <button className={cn("lg:hidden w-8 h-8 rounded-lg border flex items-center justify-center text-xs transition-all", showStarred ? "bg-warn/15 border-warn/40 text-warn" : "bg-card border-border/50 text-muted-foreground")}
                  onClick={() => { setShowStarred(!showStarred); setPage(1) }}>
                  <Star className={cn("w-4 h-4", showStarred && "fill-current")} />
                </button>
                <button className={cn("lg:hidden w-8 h-8 rounded-lg border flex items-center justify-center text-xs transition-all", showDoneOnly ? "bg-success/15 border-success/40 text-success" : "bg-card border-border/50 text-muted-foreground")}
              ...
              <>
                {isLoadingCat ? (
                  <div className="space-y-2.5">
                    {Array.from({ length: 6 }, (_, i) => <div key={i} className="h-16 rounded-xl skeleton" />)}
                  </div>
                ) : paged.length === 0 ? (
                  ...
                  <div className="space-y-2 sm:space-y-2.5">
                    {paged.map((q, i) => (
                      <QuestionCard key={q.id} question={q} index={i} isDone={doneSet.has(q.id)} isStarred={starSet.has(q.id)}
                        onToggleDone={() => toggleDone(q.id)} onToggleStar={() => toggleStar(q.id)} />
                    ))}
                  </div>
                )}
                  ...
                  <div className="flex items-center justify-center gap-1 sm:gap-1.5 mt-5 sm:mt-6 py-3">
                    ...
                    {visiblePages.map((p) => (
                      <button key={p} onClick={() => setPage(p)}
                        className={cn("min-w-[32px] sm:min-w-[36px] h-9 rounded-lg border text-xs sm:text-sm font-mono flex items-center justify-center transition-all active:scale-95",
                          page === p ? "bg-primary border-primary text-primary-foreground glow-border" : "border-border/50 bg-card text-muted-foreground hover:border-primary/30 hover:text-primary")}>
                        {p}
                      </button>
                    ))}
                    ...
                    <span className="text-muted-foreground text-[10px] sm:text-xs font-mono ml-1 sm:ml-2 hidden sm:inline">{page}/{totalPages} · {filtered.length}题</span>
                  </div>
                )}
              </>
            )}
              ...
              <>
                {quizQuestions.length === 0 ? (
                  ...
                  <div className="max-w-[680px] mx-auto" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
                    ...
                    {currentQuizQ && (
                      <motion.div key={quizIndex} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, ease: "easeOut" }}
                        className="bg-card border border-border/40 rounded-2xl overflow-hidden card-hover">
                        <div className="p-5 sm:p-7">
                          <div className="text-[15px] sm:text-[17px] leading-[1.75] font-medium text-foreground">{currentQuizQ.question}</div>
                        </div>
                              ...
                              <div className="px-5 sm:px-7 pb-5 sm:pb-7">
                                <div className="answer-container p-4 sm:p-5">
                                  <div className="flex items-center gap-1.5 text-success/80 text-[10px] sm:text-[11px] font-bold uppercase tracking-wider mb-2.5">
                                    <span className="w-1 h-1 rounded-full bg-success/60" /> 参考答案
                                  </div>
                                  <div className="text-[13px] sm:text-[14px] leading-[1.9] whitespace-pre-wrap">{currentQuizQ.answer}</div>
                                </div>
                              </div>
                            </motion.div>
                          )}
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-4">
                      <button disabled={quizIndex === 0} onClick={() => { setQuizIndex((i) => i - 1); setQuizShowAnswer(false) }}
                      ...
                      {!quizShowAnswer ? (
                        <button onClick={() => setQuizShowAnswer(true)}
                        ...
                        <button onClick={() => currentQuizQ && toggleDone(currentQuizQ.id)}
                          className={cn("px-4 py-3 rounded-xl text-xs sm:text-sm font-medium transition-all active:scale-95",
                            currentQuizQ && doneSet.has(currentQuizQ.id) ? "bg-success/15 text-success border border-success/25" : "bg-card border border-border/40 text-muted-foreground hover:border-primary/25")}>
                          {currentQuizQ && doneSet.has(currentQuizQ.id) ? "✓ 已掌握" : "标记掌握"}
                        </button>
                      )}
                      <button disabled={quizIndex >= quizQuestions.length - 1} onClick={() => { setQuizIndex((i) => i + 1); setQuizShowAnswer(false) }}
                        className="px-4 py-3 rounded-xl border border-border/40 bg-card text-muted-foreground text-xs sm:text-sm font-medium transition-all hover:border-primary/25 hover:text-foreground disabled:opacity-20 disabled:cursor-not-allowed active:scale-95">
                        下一题 →
                      </button>
                    </div>
      ...
      <BottomNav mode={mode} showStarred={showStarred} showDoneOnly={showDoneOnly} onAll={goAll} onStarred={goStarred} onDone={goDone} onQuiz={startQuiz} />
      <CategoryDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} categories={categories} current={currentCat} onSelect={(n) => { switchCat(n); setDrawerOpen(false) }} onRandomQuiz={startQuiz} onReset={resetProgress} />
    </div>
  )
}
